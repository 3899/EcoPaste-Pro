...
本项目为 [EcoPasteHub/EcoPaste](https://github.com/EcoPasteHub/EcoPaste) 的 Fork 增强版本，在官方原版基础上，重构并新增了跨设备数据同步、WebDAV 云端数据资产安全备份、全景存储统计等多项高级功能，同时全面增强了剪贴板内容的智能分类识别引擎与日常交互的沉浸式体验。

---

## 📌 Pro.5.x 系列

> 基于原版 `v0.6.0-beta.3` 分支深度重构与功能演进

### Pro.5.7

#### 🐞 Bug 修复
- 修复侧边导航模式下历史记录编辑功能无响应的问题。
- 修复激活切换全部分组机制在窗口隐藏后失效的问题，场景兼容性提升。
- 修复 WebDAV 轻量备份空字段报错问题，兼容 NULL 历史记录，避免备份意外中断。
- 修复 WebDAV 最大备份数对自动备份不生效问题，统一清理逻辑，避免并发重复删除。
- 修复剪贴板主窗口设置按钮无响应问题，恢复顶部与侧边导航布局的偏好设置入口。
...
#### ✨ 新增功能
- **🖱️ 鼠标中键全局快捷控制**：支持单击、长按中键快速开关剪贴板，可自定义触发方式与长按时长以防止误触；不影响系统原生功能，关闭后自动释放资源，窗口内操作不会误收窗口。
- **⌨️ 全新快捷键录制与推荐**：升级快捷键设置界面，支持一键录制快捷键，点击下拉按钮可查看常用快捷键与修饰键建议面板。
- **🔒 Win+V 全局无缝接管**：完美兼容 Windows 系统剪贴板，无需修改系统配置，一键接管 / 自动恢复原生 Win+V 功能，非侵入式体验。

#### 💫 体验优化
- **📌 窗口钉住功能全面优化**：升级窗口固定驻留能力，顶栏、侧边导航两种布局下，重新打开窗口均可保留钉住状态。同时系统可识别窗口钉住状态，在 Windows 不夺焦模式下，点击窗口外部区域不会自动关闭。
- **🔒 Windows 不夺焦静默粘贴模式极致体验升级**：
  - **新增 `空格键` 激活搜索**：非激活状态下按下空格键，能够快速激活搜索框并录入内容。
  - **新增 `ESC 键` 极速隐藏**：不夺焦模式下按下 `ESC` 键可一键隐藏窗口并退回后台。
  - **精简 `退格键` & `删除键` 拦截**：在非激活状态下彻底移除了对退格、删除键的拦截，避免编辑内容时误删剪贴板数据，保障数据安全。
- **📐 扩充侧边导航模式拖拽区域**：侧边导航布局中，侧边栏、窗口顶栏、搜索及标签区域均可直接拖拽移动。

#### 🐞 Bug 修复
- **🧹 修复焦点与通知事件内存泄漏问题**：优化组件销毁时的事件清理逻辑，切换导航模式、重载布局后，可正常释放各类事件监听，彻底解决监听堆积、后台运行异常及内存泄漏问题。
...
#### ✨ 新增功能
- **📐 剪贴板“侧边导航”布局上线**：在「偏好设置 - 通用设置 - 外观设置 - 视图模式」中新增侧边导航选项。开启后，剪贴板窗口左侧将展示窄款侧边栏，纵向排布应用 Logo、八大分类图标、收藏夹与设置按钮；右侧搭配通栏搜索框，界面布局更合理，操作体验更沉浸高效。
- **🏷️ 收藏夹类型标签过滤与配置**：侧边导航模式下，点击收藏按钮即可在页面顶部调出分类筛选栏。同时新增收藏夹类型标签显示开关，支持自主勾选文本、图片、链接等标签，按需过滤内容。
- **📋 收藏夹排序**：在「偏好设置 - 剪贴板 - 排序设置」分组，新增默认开启的「收藏夹排序」开关，开启后可通过右键菜单（置顶、上移、下移、置底）或卡片顶部专属拖拽手柄调整条目顺序。收藏夹排序功能与常规分组的自动排序、时间线置顶规则完全解耦，实现互不干扰的双轨排序能力。
- **🔍 搜索支持空格分词与通配符**：重构搜索检索逻辑，支持空格分隔多个关键词实现交集查询；同时兼容 `*`、`?` 通配符语法，有效提升内容检索的精准度与灵活性。
- **🔒 新增 Windows 不夺焦静默粘贴模式**：在「偏好设置 - 剪贴板 - 窗口设置」新增该功能开关，开启后将自动关闭窗口默认聚焦。唤醒剪贴板时不会抢占当前窗口焦点，可实现文件重命名、双击粘贴等无干扰的静默操作。按下左、右方向键、`Ctrl+F`、`/` 快捷键，或直接点击搜索框，窗口会自动激活并获取焦点，兼顾静默粘贴与高效检索的使用需求。

#### 💫 体验优化
- **🗃️ 优化大数据批量删除体验**：重构批量删除、自动清理的底层查询逻辑，采用轻量化数据检索方案，删除时不再加载文本、图片等大体积字段，大幅降低内存占用，彻底解决海量数据清理时界面卡顿、假死的问题。
- **⚡ 搜索输入增加防抖处理**：搜索框配置 `300ms` 防抖延时，避免连续输入时频繁发起数据库查询，彻底解决打字过程中界面掉帧、卡顿问题。
- **🖼️ 图片加载全面优化**：图片组件启用原生懒加载特性，并新增后台多线程异步预解码队列。可视区外图片提前完成解码与缓存，滚动页面时消除白块闪烁，实现流畅顺滑的浏览体验。

#### 🏗️ 架构优化
- **🧹 新增数据库智能整理机制**：数据删除后自动执行查询计划优化，提升数据库运行效率；当单次批量删除或自动清理条目数量≥50 条时，自动触发数据库收缩，及时释放闲置磁盘空间。
- **🔒 增加独占式维护锁**：通过单例锁机制串行执行数据库优化、收缩等维护操作，有效避免高并发删除场景下出现数据库锁表、写入冲突，保障后台任务稳定运行。
- **🗄️ 数据库索引深度优化**：为剪贴板数据表的高频筛选、排序字段新建多组索引，规避全表扫描与排序开销，大幅提升大数据量下列表加载、分类切换的检索响应速度。
...
#### ✨ 新增功能（全新存储空间管家体验）
- **📊 全景图表视界**
  - **📋 多维存储统计面板**：新增 12 大核心内容类型存储统计，直观展示空间占用情况；支持「全部 / 收藏」独立切换，可单独查看收藏记录占用数据。
  - **⚡ 超高性能统计引擎**：重构物理体积溯源与大盘统计算法，移除阻塞性磁盘 IO 遍历，统一改为内存级数据库查询，上万条记录、数百 GB 数据也能瞬时加载。
...
#### 🐞 Bug 修复
- **🌐 网页链接识别修复**：优化剪贴板类型判断逻辑，纯文本格式的 URL / 邮箱会强制识别为纯文本，解决从网页复制带样式链接被误判为 HTML 的问题。
- **🎨 颜色代码误判修复**：重构颜色识别校验引擎，仅标准色值，避免普通英文单词（如 `Tan`、`azure` 等）被误判高亮。

#### 🐛 原版缺陷修复
- **📂 文件图标路径隔离修复**：深度对接 `tauri-plugin-fs-pro` 底层 API，文件图标将精准保存至用户自定义目录，不再残留到系统默认 `AppData` 路径。
...
- **📰 复合型代码误判**：彻底修复从 IDE 开发环境或网页中复制同时携带“纯文本代码 + HTML 富文本语义”混合片段时，内容被强制识别为 `HTML` 超文本类型的底层优先级越权漏洞，确保复制的代码内容可被精准识别归类。
...
#### 🐛 原版缺陷修复
- **👁️ 条目头部全宽释放**：取消了原版界面中为右侧隐形操作按钮强行预留的固定宽度行为，未选中状态下的记录头部属性栏可实现 100% 全宽显示，内容展示空间更充足、信息呈现更完整。
...
#### ✨ 新增功能
- **🎨 CMYK 颜色识别**：新增 CMYK 颜色格式的智能提取与预览支持。
...
- **☁️ WebDAV 云备份体系完善**：
  ...
  - **自动调度引擎**：内置独立的前端任务调度引擎，实现了灵活的“定时”、“间隔”以及“Cron 表达式”三种循环模式，实现完整备份与精简备份双线并行调度，提升备份效率。
- **📝 新增 Markdown 支持**：采用全新计分权重正则检测法，精准判断 Markdown 格式，彻底规避普通文本、代码被误判归类的问题，同时支持 Markdown 格式化沉浸编辑，优化编辑体验。
- **🖼️ 图片定位目录**：图片类型支持一键拉起资源管理器精准定位原文件所在目录。

#### 💫 体验优化
- **☁️ 无缝恢复交互**：重构 WebDAV 恢复弹窗唤醒机制，会优先即刻渲染骨架及 Loading UI，后台采用非阻塞式拉取云端记录，彻底解决网络延迟导致的界面假死、无反馈顿挫问题，提升操作流畅度。
